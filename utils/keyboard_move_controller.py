...
def _get_direction_keys(direction):
    direction_map = {
        Direction.UP: [Key.up],
...
def _get_main_direction(direction):
    """获取方向的主方向（用于跑步）"""
    ...
    def move(self, target_direction, move_mode='running'):
        ...
        with self._lock:
            ...
            if move_mode == MoveMode.RUNNING:
                current_main = _get_main_direction(self.current_direction) if self.current_direction else None
                target_main = _get_main_direction(target_direction.value)
